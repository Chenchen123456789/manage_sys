...
XSU = {
  ...
  initAllCell: function (workBook, sheetName) {
    ...
    let startCell = ref[0];
    let endCell = ref[1];
    let sc = XLSXS.utils.decode_cell(startCell).c;
    let sr = XLSXS.utils.decode_cell(startCell).r;
    let ec = XLSXS.utils.decode_cell(endCell).c;
    let er = XLSXS.utils.decode_cell(endCell).r;
    let isExist;
    for (let c = sc; c <= ec; c++) { //初始化所有单元格
      for (let r = sr; r <= er; r++) {
        let temp = XLSXS.utils.encode_cell({
          c: c,
          r: r
        });
        isExist = false;
        for (let cell in workBook.Sheets[sheetName]) {
          if (cell != '!cols' && cell != '!merges' && cell != '!ref' && cell != '!fullref') {
            if (temp == cell) {
              isExist = true;
              break;
            }
  ...
  mergeCells: function (workBook, sheetName, startCell, endCell) {
    /*let sc = startCell.substr(0, 1).charCodeAt(0) - 65;
    let sr = startCell.substr(1);
    sr = parseInt(sr) - 1;
    let ec = endCell.substr(0, 1).charCodeAt(0) - 65;
    let er = endCell.substr(1)
    er = parseInt(er) - 1;*/
